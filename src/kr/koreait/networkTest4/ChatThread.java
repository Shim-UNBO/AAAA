...
//	서버에 접속된 모든 클라이언트의 목록을 가지고 있어야 특정 클라이언트에서 전송된 메시지를 모든 클라이언트로 전송할 수 있다.
//	서버에 접속된 모든 클라이언트의 목록은 서버에 접속된 모든 클라이언트가 공유해야 하므로 static 멤버로 만든다.
	static List<PrintWriter> chatList = Collections.synchronizedList(new ArrayList<PrintWriter>());
	Socket socket;
	PrintWriter printWriter;
	
	public ChatThread() { }
//	클라이언트가 접속될 때 마다 생성되는 소켓을 인수로 받아 PrintWriter 클래스 객체를 생성해 서버에 접속한 모든 클라이언트의 목록을 기억하는
//	List 인터페이스 객체 chatList에 저장하는 생성자
...
//			입력한 메시지를 서버에 접속된 모든 클라이언트로 전송한다.
			while(true) {
...
//				접속한 모든 클라이언트로 메시지를 전송한다.
			...
			//finally 블록으로 왔다는 것은 서버에 접속한 특정 클라이언트가 채팅 창을 닫았거나 오류로 인해 채팅 창이 중지된 경우이므로 채팅
			//목록에서 제거한다.
			chatList.remove(printWriter);
			//자신을 제외한 나머지 모든 접속자에게 채팅을 종료 했다는 메시지를 전송한다.
			for(PrintWriter pw : chatList) {
				pw.write("#" + name + "# 님이 나갔습니다.\n");
				pw.flush();
			}
