...
//					채팅에 사용한 모든 객체를 닫는다.
		...
		add(textArea);
	      ...
	      panel.add(textField);
	      panel.add(button);
	      panel.setBackground(new Color(new Random().nextInt(16777216)));
	      button.setBackground(new Color(new Random().nextInt(16777216)));
	      add(panel, BorderLayout.SOUTH);
	      //텍스트 필드와 전송버튼에 ActionListener을 걸어준다.
...
//			클라이언트가 접속했으므로 텍스트 필드와 전송 버튼을 활성화 시키과 메시지를 입력할 수 있게 텍스트 필드로 포커스를 이동시킨다.
...
//	클라이언트에서 언제 메시지를 보내올지 모르기 때문에 통신이 종료될 때 까지 반복하며 클라이언트에서 전송되는 메시지를 스레드를 
//	실행해서 받는다.
	@Override
	public void run() {
